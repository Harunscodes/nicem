...
No product code has been built yet. No experiments have been run yet. The benchmark is not yet executing. The Turkish query review (QR9) is complete. A cross-language semantic equivalence audit (EN/NL/TR) is complete and PASS for internal Stage 1 use. TM1 is **confirmed**: v0.1 uses the OpenAI GPT-4.1-mini/GPT-4.1 family (one family, one tokenizer). **Stage 1a (tokenizer-only sanity gate) is unblocked** — the remaining step is a tooling task (confirm the exact `tiktoken` encoding name and write the counting script); Stage 1a requires no API calls.
