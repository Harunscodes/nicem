...
No product code has been built yet. No experiments have been run yet. The benchmark is not yet executing. The Turkish query review (QR9) is complete. A cross-language semantic equivalence audit (EN/NL/TR) is complete and PASS for internal Stage 1 use. The TM1 decision note proposes the OpenAI GPT-4.1-mini/GPT-4.1 family (one family, one tokenizer) for v0.1; the only step remaining before Stage 1 (tokenizer-only sanity gate) is project-owner confirmation of that recommendation.
