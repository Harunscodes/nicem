...
14. **Implement and run Stage 2-local LLM rehearsal** — install Ollama (or chosen local provider); pull a small instruction model; add `can_run_local_mode`, `--local`/`--confirm-local` flags, and provider-aware output routing to the runner; code-review local additions; re-run dry-run validation (11/11 PASS); set `allow_local_calls=True`; run `--local --confirm-local` for S2-INT-004-en-A only; inspect logs under `results/stage2-local/`; set `allow_local_calls=False` again. See `stage2-local-llm-rehearsal-plan.md` (s2-local-rehearsal-v0.1.0).
