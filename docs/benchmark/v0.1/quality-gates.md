...
| Version-pinned model IDs set (TM1-b/c) | **PASS** | CONFIRMED 2026-06-14 from official OpenAI sources: `response_model_id = gpt-4.1-mini-2025-04-14`; `embedding_model_version = not-exposed-by-provider`; set in runner CONFIG. Reconfirm against live `/v1/models` at run time |
...
| `pricing_version` + pricing table recorded | **PASS** | CONFIRMED 2026-06-14: `pricing_version = openai-2026-06-14`; gpt-4.1-mini input $0.40/1M (0.00040/1K), output $1.60/1M (0.00160/1K); text-embedding-3-small $0.02/1M (0.00002/1K); set in runner CONFIG; `pricing_configured()` returns True |
...
| Local LLM rehearsal plan | **COMPLETE** | `stage2-local-llm-rehearsal-plan.md` (s2-local-rehearsal-v0.1.0, 2026-06-15): 15-section zero-cost rehearsal phase before Stage 2-live; provider options (Ollama recommended); candidate models; scope (first run = S2-INT-004-en-A); separation from Stage 2-live (`results/stage2-local/`); runner changes required; embedding strategy; logging requirements; safety guards (local must not call OpenAI, `allow_api_calls` stays False); evaluation label (`LOCAL_REHEARSAL_ONLY`); first local run approval criteria; relationship to NiceM evidence |
| Local provider guard implemented | **IMPLEMENTED, BLOCKED** | `can_run_local_mode()` added to runner (2026-06-15); `allow_local_calls=False`; `--local`/`--confirm-local`/`--max-runs` CLI flags; `LOCAL_RESULTS_DIR = results/stage2-local/`; `run_local()` (Agent A only; Agent B blocked); 6 local guard tests (all PASS); `allow_local_calls` stays `False` |
...
**Stage 2 gate verdict: BLOCKED** — all five decisions are CONFIRMED; the run plan, the live-run readiness document, the model/pricing config (CONFIRMED), and the runner all exist. The version-pinned model IDs (TM1-b/c), `pricing_version`, and pricing table are CONFIRMED from official OpenAI sources and set in CONFIG. The live completion + embedding paths and the `BudgetGuard`-driven `run_live` loop are now **implemented** (2026-06-14, review pending). The dry-run passes all structural validations (30 runs, all required fields, $0 cost, no API key, pricing self-test PASS) and 6 safety-guard tests PASS. Stage 2 is still not runnable: the live paths require code review; `allow_api_calls` is False (the single config flip that unblocks live mode); `OPENAI_API_KEY` is not provided; and `--live --confirm-spend` are required. Live mode remains refused by the strict guard. All remaining blockers and the manual approval checklist are tracked in `stage2-live-run-readiness.md` (s2-live-readiness-v0.1.2).
...
| Local LLM rehearsal plan | COMPLETE 2026-06-15 | `stage2-local-llm-rehearsal-plan.md` (s2-local-rehearsal-v0.1.0): zero-cost pre-live rehearsal; Ollama recommended; 15 sections; `results/stage2-local/` directory separate from Stage 2-live; `allow_api_calls` stays False throughout |
| Local provider guard in runner | IMPLEMENTED 2026-06-15, BLOCKED | `can_run_local_mode()`, `allow_local_calls=False`, `--local`/`--confirm-local`/`--max-runs`, `LOCAL_RESULTS_DIR`, `run_local()` (Agent A only); 12 guard tests PASS (6 OpenAI + 6 local); no localhost call made |
| First-run selector in runner | IMPLEMENTED 2026-06-15, BLOCKED | `--intent-id`, `--language`, `--agent` flags; `first_run_only=True`; `_check_first_run_selector()`; `_resolve_run_id()`; 5 selector guard tests PASS (17/17 total); no run executed |
| Manual approval checklist for live run | PENDING_PROJECT_OWNER | `stage2-live-run-readiness.md` §8 + `stage2-live-code-review.md` §12; 9-item checklist; must be fully checked before `allow_api_calls=True`; local rehearsal recommended first |
...
| Stage 2 smoke test | **NO** | Decisions CONFIRMED + run plan + readiness doc + model/pricing CONFIRMED + runner with implemented live paths + `BudgetGuard`-driven `run_live` loop + self-test + 6 guard tests (2026-06-14); remaining: code-review the live paths, provide `OPENAI_API_KEY`, complete the approval checklist, set `allow_api_calls=True`, run `--live --confirm-spend` |
...
11. **~~Record exact model IDs (TM1-b/c), `pricing_version`, and pricing table~~ — DONE (2026-06-14).** Confirmed from official OpenAI sources: `response_model_id = gpt-4.1-mini-2025-04-14`; `embedding_model_version = not-exposed-by-provider`; `pricing_version = openai-2026-06-14`; rates 0.00040 / 0.00160 / 0.00002 per 1K. Set in runner CONFIG via named constants; dry-run revalidated 11/11 PASS. See `stage2-model-pricing-config.md` (s2-model-pricing-v0.1.1) §8. Reconfirm snapshot + rates against the live API immediately before the run.
...
14a. **~~Implement local provider guard and first-run selector in runner~~ — DONE (2026-06-15).** `can_run_local_mode()`, `allow_local_calls=False`, `--local`/`--confirm-local`/`--max-runs`/`--intent-id`/`--language`/`--agent` CLI flags, `first_run_only=True`, `_check_first_run_selector()`, `_resolve_run_id()`, `LOCAL_RESULTS_DIR`, `run_local()` (Agent A only; Agent B blocked), 17/17 guard tests PASS (6 OpenAI + 6 local + 5 selector). No localhost call, `results/stage2-local/` absent.

14b. **Run Stage 2-local rehearsal** — install Ollama; pull `llama3.2:3b-instruct`; confirm `http://localhost:11434/v1` reachable; set `allow_local_calls=True`; run `--local --confirm-local --max-runs 1`; inspect `results/stage2-local/raw_outputs/S2-INT-004-en-A.json`; set `allow_local_calls=False` again. See `stage2-local-llm-rehearsal-plan.md` §15.

15. **Complete the approval checklist and enable OpenAI live mode** — project owner reviews `stage2-live-code-review.md` §4–§11 and checks all 9 items in §12; reconfirms `response_model_id` against live `/v1/models`; reconfirms pricing currency; confirms `openai` SDK version; sets `allow_api_calls=True`; exports `OPENAI_API_KEY`; runs `--live --confirm-spend` starting with one English Agent A run (S2-INT-004-en-A); manually inspects raw output and `budget_state.json` before continuing. No paid API call until all 9 checklist items are checked.
