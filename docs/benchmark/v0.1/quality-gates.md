...
| Local provider guard in runner | IMPLEMENTED 2026-06-15, BLOCKED | `can_run_local_mode()`, `allow_local_calls=False`, `--local`/`--confirm-local`/`--max-runs`, `LOCAL_RESULTS_DIR`, `run_local()` (Agent A only); 6 new guard tests PASS (12/12 total); no localhost call made |
...
14a. **~~Implement local provider guard in runner~~ — DONE (2026-06-15).** `can_run_local_mode()`, `allow_local_calls=False`, `--local`/`--confirm-local`/`--max-runs` CLI flags, `LOCAL_RESULTS_DIR`, `_get_local_client()`, `run_local()` (Agent A only; Agent B blocked), 6 local guard tests (all PASS). Dry-run: 11/11 PASS, 30 runs, 12/12 guard tests PASS, no localhost call, `results/stage2-local/` absent.
