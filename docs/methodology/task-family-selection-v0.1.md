...
- **How many synthetic documents?** A single knowledge base document reduces noise (all agents access the same information). Multiple documents increase ecological validity. For v0.1, a single coherent synthetic knowledge base with 4–6 policy sections is likely sufficient.
- **How many task instances?** Enough to estimate execution-cost distributions with some reliability across language conditions. A rough starting range is 20–50 instances per language condition for a pilot; the exact number depends on the statistical power calculation (M8, still open).
- **How many languages?** At minimum three: English (low-premium baseline), one mid-premium language (e.g., Dutch, Portuguese — also useful as a sanity check), and one high-premium language (e.g., Turkish, Arabic) from the Petrov/Ahia token-tax spectrum. Four or five languages would give richer signal but increase construction cost.
