...
- [x] **Decide human vs. automated evaluation** — resolved: `docs/methodology/evaluation-method-v0.1.md` defines the hybrid conservative method — deterministic/semi-deterministic fact-set checks primary, project-owner human audit layer (all UNCERTAINs mandatory, FAILs recommended, sampled PASSes), Turkish self-review permitted (mother tongue), LLM-as-judge optional support only and never sole source of truth; independent bilingual review required before public claims.
